    ...
    public void onScanningDone(RecognitionResults results) {
        ...
        for (BaseRecognitionResult r : dataArray) {
            if (r instanceof BarDecoderScanResult) { // r is barcode scan result
                ...
                Pattern p = Pattern.compile("((M\\s*i\\s*ss)|(M\\s*r)|(M\\s*r|M\\s*r\\s*s))\\s+");
                Pattern p2 = Pattern.compile("(([0-9])|([0-2][0-9])|([3][0-1]))\\s*(Jan\\.*|Feb\\.*|Mar\\.*|Apr\\.*|May|Jun\\.*|Jul\\.*|Aug\\.*|Sep\\.*|Oct\\.*|Nov\\.*|Dec\\.*)\\s*(\\d{4})");
                String sex="";
                String year="";

                int count = 0;
                Matcher m = p.matcher(ocrResult.toString());
                Matcher m2 = p2.matcher(ocrResult.toString());
